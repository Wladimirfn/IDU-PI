...
test("main and installation menus render unified control options", () => {
	...
	assert.match(formatInstallationMenu(), /6\. Trigger supervisor/u);
	assert.match(formatInstallationMenu(), /7\. ← Volver/u);
...
test("wizard source avoids AgentLabs scans prepare and bootstrap", () => {
	...
	// now in src/cli/wizard/helpers.ts. Read from there.
