/**
 * cli-command-catalog.test.ts — pins the command catalog of src/cli.ts.
 *
...
function parseCaseLabels(src: string): string[] {
	...
	for (const line of lines) {
		...
		if (inSwitch) {
			...
			// A label is `case "X":`. We collect all of them inside the
			// switch (any depth). The default case has no label.
			const m = line.match(/^\s*case\s+"([^"]+)"\s*:/);
			if (m) labels.push(m[1]);
			// Exit when the OUTER switch brace closes (depth returns to 0).
