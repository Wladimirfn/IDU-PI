...
test("stuck_tasks_1h con task_stuck ageMs >= 1h sin task_created posterior inyecta 1 envelope", () => {
	...
	try {
		appendEvent(stateRoot, {
			...
			kind: "task_stuck",
			projectId: "idu-pi",
			payload: { taskId: "t-1", ageMs: 3_700_000, domain: "stale_work", severity: "warning" },
...
test("Idempotencia: 2 ticks consecutivos con misma ventana no duplican", () => {
	...
	try {
		appendEvent(stateRoot, {
			...
			kind: "task_stuck",
			projectId: "idu-pi",
			payload: { taskId: "t-2", ageMs: 3_700_000, domain: "stale_work", severity: "warning" },
			sourceRef: "autonomous-alert-engine",
			evidenceRefs: [],
		});
		const r1 = runTriggerEngineTick({ stateRoot, projectId: "idu-pi", now: new Date(FIXED_TS) });
		const r2 = runTriggerEngineTick({ stateRoot, projectId: "idu-pi", now: new Date(FIXED_TS) });
...
test("stuck_tasks_1h con task_created posterior a task_stuck no matchea", () => {
	...
	try {
		appendEvent(stateRoot, {
			...
			kind: "task_stuck",
			projectId: "idu-pi",
			payload: { taskId: "t-3", ageMs: 3_700_000, domain: "stale_work", severity: "warning" },
			sourceRef: "autonomous-alert-engine",
			evidenceRefs: [],
		});
		appendEvent(stateRoot, {
...
test("intention_decision_pending con ageMs >= 30min y requiresHuman=true inyecta warning", () => {
	...
	try {
		appendEvent(stateRoot, {
			...
			projectId: "idu-pi",
			payload: { request: "touch auth.ts", ageMs: 1_900_000, requiresHuman: true },
...
test("intention_decision_pending con requiresHuman=false no matchea", () => {
	...
	try {
		appendEvent(stateRoot, {
			...
			kind: "intention_decision_pending",
			projectId: "idu-pi",
			payload: { request: "tweak copy", ageMs: 1_900_000, requiresHuman: false },
			sourceRef: "project-preflight",
			evidenceRefs: [],
		});
		const result = runTriggerEngineTick({
		...
		const pending = readPendingInjections(stateRoot);
		assert.equal(
			pending.filter((p) => p.triggerId === "intention_decision_pending").length,
			0,
		);
	} finally {
		cleanup();
...
test("Triggers con ventana vacía no inyectan", () => {
	...
	try {
		...
		assert.equal(
			pending.filter(
				(p) => p.triggerId === "stuck_tasks_1h" || p.triggerId === "intention_decision_pending",
			).length,
			0,
		);
	} finally {
