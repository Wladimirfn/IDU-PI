...
| `idu_source_read` | Lee contenido acotado de fuente registrada; PDFs quedan metadata-only. |
| `idu_source_extract` | Extrae/actualiza texto acotado para `.md/.txt`; PDFs quedan metadata-only sin OCR. |
| `idu_source_report` | Reporta metadata, hash, estado de extracción y limitaciones de una fuente. |
