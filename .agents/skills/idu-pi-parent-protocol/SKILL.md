...
4. **Wait & Monitor**: If `idu_delegate` is called with `async: true`, the worker runs in a detached daemon. Stay blocked and stream live logs by running terminal command: `idu wait <run_id> --follow` (or `node dist/src/cli.js wait <run_id> --follow`). NEVER end your turn saying "I'll wait" or "checking later". You can also use `idu_worker_wait` or `idu_worker_status`.
