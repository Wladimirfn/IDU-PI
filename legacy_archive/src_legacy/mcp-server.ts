...
import { CrossCliProcessManager } from "./cross-cli/process-manager.js";
import {
...
const CROSS_CLI_TOOLS: IduMcpToolDefinition[] = [
	{
		name: "idu_delegate" as IduMcpToolName,
		description:
		...
		inputSchema: {
			type: "object",
			properties: {
				task: { type: "string", description: "Instructions and prompt for the worker." },
				profile: { type: "string", description: "Target profile from ~/.idu/profiles.json." },
				working_dir: { type: "string", description: "Working directory." },
				async: { type: "boolean", description: "Whether to run async in background." },
	...
	{
		name: "idu_delegate_parallel" as IduMcpToolName,
	...
	{
		name: "idu_worker_status" as IduMcpToolName,
		description: "Checks real-time execution status and recent logs of a delegated run_id.",
		inputSchema: {
			type: "object",
			properties: {
	...
	{
		name: "idu_worker_result" as IduMcpToolName,
		description: "Retrieves complete result, output summary, and log transcript for a run_id.",
		inputSchema: {
			type: "object",
			properties: {
				run_id: { type: "string", description: "Unique run ID." },
	...
	{
		name: "idu_capabilities" as IduMcpToolName,
		description: "Lists available worker profiles and detected installed terminal CLIs.",
		inputSchema: {
			type: "object",
			properties: {},
		},
...
const LEAN_ALLOWED_TOOLS = new Set([
	"idu_status",
	"idu_project_status",
	"idu_preflight",
	"idu_postflight",
...
export function listIduMcpTools(): IduMcpToolDefinition[] {
	const filteredLegacy = TOOLS.filter((t) => LEAN_ALLOWED_TOOLS.has(t.name));
	return [...filteredLegacy, ...CROSS_CLI_TOOLS].map((toolDefinition) => ({ ...toolDefinition }));
}
	...
	switch (request.method) {
		...
		case "tools/call": {
			...
			if (name === "idu_delegate") {
				...
				const res = await CrossCliProcessManager.getInstance().delegate(
					{
						task: String(toolArgs.task || ""),
						profile: String(toolArgs.profile || ""),
						workingDir: toolArgs.working_dir ? String(toolArgs.working_dir) : undefined,
						parentOrchestrator: "orchestrator",
						timeoutMs: typeof toolArgs.timeout_ms === "number" ? toolArgs.timeout_ms : undefined,
						contextFiles: Array.isArray(toolArgs.context_files) ? toolArgs.context_files.map(String) : [],
					},
					Boolean(toolArgs.async),
				);
				return jsonRpcResult(request.id, {
					content: [{ type: "text", text: `${JSON.stringify(res, null, 2)}\n` }],
					isError: res.status === "failed" || res.status === "timeout",
				});
			}
			if (name === "idu_delegate_parallel") {
				const toolArgs = asRecord(params.arguments);
				const tasks = Array.isArray(toolArgs.tasks) ? (toolArgs.tasks as Array<Record<string, unknown>>) : [];
				const promises = tasks.map((t) =>
					CrossCliProcessManager.getInstance().delegate(
						{
							task: String(t.task || ""),
			...
			if (name === "idu_worker_status") {
				const toolArgs = asRecord(params.arguments);
				const st = CrossCliProcessManager.getInstance().getStatus(String(toolArgs.run_id || ""));
				return jsonRpcResult(request.id, {
			...
			if (name === "idu_worker_result") {
				const toolArgs = asRecord(params.arguments);
				const res = CrossCliProcessManager.getInstance().getResult(String(toolArgs.run_id || ""));
				return jsonRpcResult(request.id, {
					content: [{ type: "text", text: `${JSON.stringify(res, null, 2)}\n` }],
					isError: !res,
				});
