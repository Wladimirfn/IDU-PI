...
test("disableSupervisorTrigger with source:'tui' writes the file and reports enabled:false", () => {
...
test("disableSupervisorTrigger is idempotent (call twice → still disabled, no throw)", () => {
	const stateRoot = makeStateRoot();
	try {
...
test("toggling disable→enable→disable ends with file showing enabled:false", () => {
	const stateRoot = makeStateRoot();
	try {
		disableSupervisorTrigger(stateRoot, {
			source: "tui",
		...
		const after1 = JSON.parse(
			readFileSync(join(stateRoot, "supervisor-trigger.json"), "utf8"),
		) as { enabled: boolean };
